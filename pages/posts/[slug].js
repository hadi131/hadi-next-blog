    ...
    <Formate>
      <section className="singleBlogMainSection" style={{ marginTop: "120px" }}>
        <div className="singleBlogSubMain">
          ...
          <div className="post">
            <h1 className="singlePostTtitle">{Single.singleBlog.title}</h1>
            <p className="">{Single.singleBlog.subTitle}</p>
      ...
      <h1 className="font-bold text-4xl py-12" style={{ padding: "20px" }}>
        Related
      </h1>
...
export async function getServerSideProps({ params }) {
  const response = await fetch(
    `https://hadi-blogging-app.vercel.app/api/blog/${params.slug}`
  );
